...
from tests.src.supasaas import SupabaseClient
...
def test_log_info_with_args_and_kwargs(client):
    ...
    with patch.object(logging, "info") as mock_info:
        ...
        mock_info.assert_called_once_with(
            f"{action} returned arg1, arg2, key1=value1, key2=value2"
        )
...
def test_log_info_with_only_kwargs(client):
    ...
    with patch.object(logging, "info") as mock_info:
        ...
        mock_info.assert_called_once_with(
            f"{action} returned key1=value1, key2=value2"
        )
...
def test_validate_type_invalid_value_type(client):
    with pytest.raises(TypeError) as exc_info:
        client._validate_type(
            value="42", name="age", is_type=int, allow_none=False
        )
...
def test_validate_type_none_value_not_allowed(client):
    with pytest.raises(ValueError) as exc_info:
        client._validate_type(
            value=None, name="age", is_type=int, allow_none=False
        )
...
def test_validate_type_is_type_none(client):
    with pytest.raises(TypeError) as exc_info:
        client._validate_type(
            value=42, name="age", is_type=None, allow_none=False
        )
